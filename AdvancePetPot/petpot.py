...
data_push_interval = datetime.timedelta(minutes=5)
...
while running:
    ...
    if current_time - last_other_readings_time >= data_push_interval:
        try:
            insert_query = "INSERT INTO SensorData (DateTime, Temperature, Humidity, SoilMoisture, AirQuality, UVIndex, LightIndex) VALUES (%s, %s, %s, %s, %s, %s, %s)"
            data = (current_time, Temperature, Humidity, ((4096-SoilMoisture) / 4096) * 100, AirQuality, UVIndex, Light)
            cursor.execute(insert_query, data)
            conn.commit()
            last_other_readings_time = current_time  # Update the last data push time
